...
@section('title', 'Edit Kelas - SMK IT Ihsanul Fikri')
        ...
        <div class="card stats-card">
            <div class="card-body">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h2 class="mb-2">
                            <i class="fas fa-edit me-2"></i>Edit Kelas: {{ $class->name }}
                        </h2>
                        <p class="mb-0">Perbarui informasi kelas</p>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('grade') is-invalid @enderror" id="grade" name="grade" required>
                                ...
                                @foreach($grades as $grade)
                                    <option value="{{ $grade }}" {{ (old('grade') ?? $class->grade) == $grade ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('class_number') is-invalid @enderror" id="class_number" name="class_number" required>
                                ...
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ (old('class_number') ?? $class->class_number) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                            @error('class_number')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            @enderror
                            <div class="form-text">
                                Siswa saat ini: {{ $class->current_students }} dari {{ $class->capacity }}
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <select class="form-select @error('homeroom_teacher') is-invalid @enderror" id="homeroom_teacher" name="homeroom_teacher">
                                ...
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->name }}" {{ (old('homeroom_teacher') ?? $class->homeroom_teacher) == $teacher->name ? 'selected' : '' }}>
                                        {{ $teacher->name }}
                                    </option>
                                @endforeach
                            </select>
